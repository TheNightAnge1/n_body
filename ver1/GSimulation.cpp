...
// static const double softeningSquared = 1e-9; // если стоит 1e-9 - энергия системы резко подскакивает
// static const double G = 6.67259e-11;
static const real_type softeningSquared = 1e-9;
static const real_type G = 6.67259e-11;
...
double count_k_energy(Particle *particles, int num_parts)
{
  ...
  // #pragma omp simd
  // ANNOTATE_SITE_BEGIN(k_energy)
  for (int k = 0; k < num_parts; k += part_size)
  {
    // ANNOTATE_ITERATION_TASK(k_energy)
    int new_n = min(num_parts, part_size + k);
...
// float count_p_energy(Particle *particles, int num_parts)
// {
//   real_type energy = 0.;
//   // ANNOTATE_SITE_BEGIN(p_energy)
//   for (int k = 0; k < num_parts; k += part_size)
//   {
//     // ANNOTATE_ITERATION_TASK(p_energy)
//     int new_n = min(num_parts, part_size + k);
//     for (int i = k; i < new_n; ++i)
//     {
//       for (int j = k; j < new_n; ++j)
//       {
//         if (i != j)
//         {
//           real_type dx = particles[j].pos[0] - particles[i].pos[0]; // 1flop
//           real_type dy = particles[j].pos[1] - particles[i].pos[1]; // 1flop
//           real_type dz = particles[j].pos[2] - particles[i].pos[2]; // 1flop

//           real_type distanceSqr = dx * dx + dy * dy + dz * dz + softeningSquared; // 6flops
//           real_type distanceInv = 1.0 / sqrt(distanceSqr);                        // 1div+1sqrt
...
//   // Have to fix
//   double energy = 0.;
//   real_type *penergies = new real_type[num_parts * num_parts];
//   // #pragma omp parallel for
//   for (int i = 0; i < num_parts; ++i)
//   {
//     // #pragma omp simd
...
//       // energy += -G * particles[j].mass * distanceInv * particles[i].mass;
//       penergies[i] = -G * particles[i].mass * distanceInv * particles[j].mass;
//     }
//   }
...
#pragma omp parallel for reduction(+ : energy)
  for (int i = 0; i < num_parts; ++i)
  {
    for (int j = 0; j < num_parts; ++j)
    {
      if (i == j)
        continue;
      float dx = particles[j].pos[0] - particles[i].pos[0]; // 1flop
      float dy = particles[j].pos[1] - particles[i].pos[1]; // 1flop
      float dz = particles[j].pos[2] - particles[i].pos[2]; // 1flop

      float distanceSqr = dx * dx + dy * dy + dz * dz + softeningSquared; // 6flops
      float distanceInv = 1.0 / sqrt(distanceSqr);                        // 1div+1sqrt
      energy += -G * particles[j].mass * distanceInv * particles[i].mass;
    }
  }
  return energy;
...
void computeAcc(Particle *src, Particle *temp, real_type step, int part_num) // part_num - число частиц
{
  ...
  // #pragma omp parallel for
  // ANNOTATE_SITE_BEGIN(accel)
  for (int k = 0; k < part_num; k += part_size)
  {
    // ANNOTATE_ITERATION_TASK(accel)
    int new_n = min(part_num, part_size + k);
    // #pragma omp simd
    for (int i = k; i < new_n; i++) // update acceleration
    {
      for (int j = k; j < new_n; j++)
      {
        if (i == j)
...
void updatePos(Particle *dist, Particle *source, Particle *newparts, real_type step, int part_num)
{
  ...
  {
    int new_n = min(part_num, part_size + k);
    for (int i = k; i < new_n; i++) // update position and velocity
    {
...
// сделать скрины с параллельной версией кода и доказать, что она куда менее эффективная, чем последовательная
